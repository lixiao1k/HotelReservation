...
public class ClientOrderListCell extends ListCell<OrderVO>{
	...
	private void showDetail(OrderVO item,MouseEvent e){
		...
		Label peopleInfo = new Label("入住人数 "+item.getPeople()+"有无儿童 ; "+(item.getChild()? "有":"无")+"儿童");
	...
	public void updateItem(OrderVO item,boolean empty){
		...
		if(item!=null){
			...
			Button review = new Button("评价");
			Button revoke = new Button("撤销");
			review.setId("rich-blue");
			...
			if(item.getStatus()==OrderStatus.EXECUTED&&!item.isCommented()){
				...
				gridPane.setHgrow(review, Priority.ALWAYS);
				gridPane.setMargin(review, new Insets(2,10,2,0));
			}
			if(item.getStatus()==OrderStatus.UNEXECUTED){
				revoke.setOnAction((ActionEvent e)->{
					Object o =DataController.getInstance().get("ClientOrderController");
					((ClientBrowseOrderListController) o).revoke(item, e);
				});
