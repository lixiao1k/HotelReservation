...
public class WebSalerOrderListCell extends ListCell<OrderVO>{
	...
	private void showDetail(OrderVO item,MouseEvent e){
		...
		Label name2 = new Label("下订单人:  "+item.getName());
	...
	public void updateItem(OrderVO item,boolean empty){
		...
		if(item!=null){
			...
			ObservableList<String> olist = FXCollections.observableArrayList("一半","全部");
			choice.setItems(olist);
			...
			if(item.getStatus()==OrderStatus.ABNORMAL){
				revoke.setOnAction((ActionEvent e)->{
					Object o =DataController.getInstance().get("WEBOrderController");
					((WebSalerCancelOrderController) o).revoke(item,choice.getValue(), e);
				});
				gridPane.add(revoke, 2, 0,1,2);
				gridPane.setHalignment(revoke, HPos.RIGHT);
				gridPane.setHgrow(revoke, Priority.ALWAYS);
				gridPane.setMargin(revoke, new Insets(2,10,2,0));
				gridPane.add(choice, 2, 0,1,2);
				gridPane.setHalignment(choice, HPos.RIGHT);
				gridPane.setHgrow(choice, Priority.ALWAYS);
				gridPane.setMargin(choice, new Insets(2,120,2,0));
			}
			setGraphic(gridPane);
		}else{
			setGraphic(null);
