    ...
    private void changePasswordAction(String password,String newPassword){
    	try{
    		...
    		if(result==UserResultMessage.FAIL_WRONGID){
    			Notifications.create().owner(clientmain.getScene().getWindow()).title("更改密码").text("错误用户！").showError();
    		}else if(result==UserResultMessage.FAIL_WRONGINFO){
    			Notifications.create().owner(clientmain.getScene().getWindow()).title("更改密码").text("原密码错误！").showError();
    		}
    		else
