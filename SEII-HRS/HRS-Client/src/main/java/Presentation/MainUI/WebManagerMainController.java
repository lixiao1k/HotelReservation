...
public class WebManagerMainController implements Initializable{
    ...
    protected void logout(ActionEvent e){
    	try {
			...
			Notifications.create().owner(webManagerMain.getScene().getWindow()).title("登出").text("登出失败！").showError();
			e2.printStackTrace();
		}
    }
    @FXML
    protected void changePassword(ActionEvent e){
		...
		btn.setOnAction((ActionEvent e3)->{
			if(password.getText()==null||password.getText().equals("\\s")
				...
				Notifications.create().owner(webManagerMain.getScene().getWindow()).title("更改密码").text("请输入密码！！").showError();
				return;
			}
    ...
    private void changePasswordAction(String password,String newPassword){
    	try{
    		...
    		if(result==UserResultMessage.FAIL_WRONGID){
    			Notifications.create().owner(webManagerMain.getScene().getWindow()).title("更改密码").text("错误用户！").showError();
    		}else if(result==UserResultMessage.FAIL_WRONGINFO){
    			Notifications.create().owner(webManagerMain.getScene().getWindow()).title("更改密码").text("原密码错误！").showError();
    		}
    		else
